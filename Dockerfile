...
RUN apt-get update -y && apt-get install -y build-essential gdb
...
# RUN parsecmgmt -a run -p fluidanimate -c gcc-pthreads -n 8 -i native
# RUN cp pkgs/apps/fluidanimate/run/out.fluid /tmp

# RUN parsecmgmt -a run -p fluidanimate -c gcc-openmp -n 8 -i native
